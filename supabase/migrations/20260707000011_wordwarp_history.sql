...
CREATE POLICY "wordwarp_select_own" ON public.wordwarp_history
  FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "wordwarp_insert_own" ON public.wordwarp_history
  FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE INDEX IF NOT EXISTS idx_wordwarp_history_user
  ON public.wordwarp_history(user_id, played_at DESC);
CREATE INDEX IF NOT EXISTS idx_wordwarp_history_score
  ON public.wordwarp_history(user_id, score DESC);
