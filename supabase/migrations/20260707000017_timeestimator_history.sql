-- Migration: timeestimator_history
...
CREATE TABLE IF NOT EXISTS public.timeestimator_history (
  ...
  avg_deviation_percent  NUMERIC(5,2),
...
CREATE POLICY "timeestimator_select_own" ON public.timeestimator_history
  FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "timeestimator_insert_own" ON public.timeestimator_history
  FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE INDEX IF NOT EXISTS idx_timeestimator_history_user
  ON public.timeestimator_history(user_id, played_at DESC);
CREATE INDEX IF NOT EXISTS idx_timeestimator_history_score
  ON public.timeestimator_history(user_id, score DESC);
