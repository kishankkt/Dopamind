...
CREATE POLICY "countflow_select_own" ON public.countflow_history
  FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "countflow_insert_own" ON public.countflow_history
  FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE INDEX IF NOT EXISTS idx_countflow_history_user
  ON public.countflow_history(user_id, played_at DESC);
CREATE INDEX IF NOT EXISTS idx_countflow_history_score
  ON public.countflow_history(user_id, score DESC);
