...
CREATE POLICY "focusgrid_select_own" ON public.focusgrid_history
  FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "focusgrid_insert_own" ON public.focusgrid_history
  FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE INDEX IF NOT EXISTS idx_focusgrid_history_user
  ON public.focusgrid_history(user_id, played_at DESC);
CREATE INDEX IF NOT EXISTS idx_focusgrid_history_score
  ON public.focusgrid_history(user_id, score DESC);
