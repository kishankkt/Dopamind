-- Migration: patternpulse_history
...
CREATE POLICY "patternpulse_select_own" ON public.patternpulse_history
  FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "patternpulse_insert_own" ON public.patternpulse_history
  FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE INDEX IF NOT EXISTS idx_patternpulse_history_user
  ON public.patternpulse_history(user_id, played_at DESC);
CREATE INDEX IF NOT EXISTS idx_patternpulse_history_score
  ON public.patternpulse_history(user_id, score DESC);
