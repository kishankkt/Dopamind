-- Migration: directiondash_history
...
CREATE POLICY "directiondash_select_own" ON public.directiondash_history
  FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "directiondash_insert_own" ON public.directiondash_history
  FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE INDEX IF NOT EXISTS idx_directiondash_history_user
  ON public.directiondash_history(user_id, played_at DESC);
CREATE INDEX IF NOT EXISTS idx_directiondash_history_score
  ON public.directiondash_history(user_id, score DESC);
